...
def _dbg_plot(t, num, ana, title):
    ...
    plt.plot(t, np.abs(ana.real - num.real), 'C0', label='Re exact')
    plt.plot(t, np.abs(ana.imag - num.imag), 'C1', label='Im exact')
...
def test_temporal_convergence(pulse):
    ...
    if DEBUG:
        print(f"\n[pulse={pulse}]  Temporal sweep (vary M):")
        for M, e in zip(Ms, errs):
            print(f"  M={M:5d}  err={e:.3e}")
        print("  drop ratios:", [f"{r:.1f}" for r in ratios])
    # crude spectral check – each refinement should reduce error ≥ 8×
    assert any(r > 5 for r in ratios), "temporal error does not decay fast enough"
