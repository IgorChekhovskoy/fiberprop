...
def PrintSpectrumInfo(A, B, n, N, M, z1, z2, t1, t2, step, dirName):
    ...
    omega_2 = fftshift(fftfreq(M - 1, step)) + central_freq  # Hz*1e15
    ...
    for j in range(n + 1):
        ...
        if j > 2:
            ...
            m += 1
        current_title = 'spectrum, single fiberprop'
        current_field = S_single
        if j < n:
            current_title = 'spectrum, {} fiberprop'.format(j)
            current_field = S[j]
        if j == n:
            m = 0
            k = 0
    ...
    for j in range(n):
        ...
        if j > 2:
            ...
            m += 1
        ax = axs[m, k]
        ax.set_title('signal spectrum in the last point, {} fiberprop'.format(j))
        ax.set_xlabel('wavelength, nm')
        ax.plot(wavelengths, S_single[0], color='black', linewidth=2, label='input')
        ax.plot(wavelengths, S[j][N - 1], color='blue', linewidth=2, label='output spectrum, {} fiberprop'.format(j))
        ax.plot(wavelengths, S_single[N - 1], color='green', linewidth=2, label='output spectrum, single fiberprop')
        ax.xaxis.set_major_locator(ticker.MultipleLocator(20))
        ax.xaxis.set_minor_locator(ticker.MultipleLocator(10))
    ...
    for j in range(n):
        ...
        if j > 2:
            ...
            m += 1
        ax = axs[m, k]
        ax.set_title('spectrum (log scale) in the last point, {} fiberprop'.format(j))
        ax.set_xlabel('wavelength, nm')
        ax.semilogy(wavelengths, S_single[0], color='black', linewidth=2, label='input')
        ax.semilogy(wavelengths, S[j][N - 1], color='blue', linewidth=2, label='output spectrum, {} fiberprop'.format(j))
        ax.semilogy(wavelengths, S_single[N - 1], color='green', linewidth=2, label='output spectrum, single fiberprop')
        ax.xaxis.set_major_locator(ticker.MultipleLocator(20))
        ax.xaxis.set_minor_locator(ticker.MultipleLocator(10))
        # ax.grid(which='major', color = 'k')
        ax.grid(which='minor', color='gray', linestyle=':')
