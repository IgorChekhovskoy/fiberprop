...
def mackey_glass(t, tau=17, n=10, beta=2, gamma=1, initial_condition=1.2):
    x = np.zeros(len(t))
    x[0] = initial_condition
    for i in range(1, len(t)):
        if i - tau < 0:
            x[i] = x[i-1] + (beta * x[i-1]**n) / (1 + x[i-1]**n) - gamma * x[i-1]
        else:
            x[i] = x[i-1] + (beta * x[i-1]**n) / (1 + x[i-1]**n) - gamma * x[i-1] + (beta * x[i-int(tau)]**n) / (1 + x[i-int(tau)]**n) - gamma * x[i-int(tau)]
    return x


def create_masks(M, size):
    np.random.seed(42)
    masks = np.random.uniform(0, 1, (M, size))
    return masks


def mackey_glass_masked(t, M, **mg_params):
    mg_series = mackey_glass(t, **mg_params)
    masks = create_masks(M, len(t))
...
def test_mcf_nn_reservoir_computing():
    ...
    solver.numerical_solution[0] = mackey_glass_masked(solver.t, solver.eq.size, **mg_params)
