...
def make_iteration(solver, Delta, phi_arr, delta_arr, Frensel_refl, plot_energy_flag=False, cos_flag='no_cos'):
    ...
    if plot_energy_flag:
        chosen_core = 3
        my_energies = {'last iteration -- backward energy': ref_array[chosen_core, :],
                       'last iteration -- forward energy': solver.energy[chosen_core, :]}
...
def mcf_resonator_simulation():
    ...
    plot2D_dict(fftshift(solver.omega/(2*pi)), my_spectrum_phase,
                xlabel='$\\omega / (2 \\pi)$', ylabel='phase($A^-_2 (\\omega, z=0)$)')
    ...
    plot2D_dict(fftshift(solver.omega/(2*pi)), my_spectrum_magnitude,
                xlabel='$\\omega / (2 \\pi)$', ylabel='$|A^-_2 (\\omega, z=0)|$')
    ...
    all_magnitudes = {f'{N_iter + 1}-iteration: {idx}-core': abs(current_solution[:, idx, solver.com.M//2+1])
                      for idx in range(solver.eq.size)}
    ...
    all_energies = {f'{N_iter + 1}-iteration: {core_idx}-core':
                        [get_energy_Simpson(current_solution[z_idx, core_idx, :], solver.com.tau)
