...
def VolumetricPowerField(dirName, Z, T, A, B):
    ...
    ax0.set_title('power [W] of {} core'.format(n1))
    ...
    ax1.set_title('power [W] of {} core'.format(n2))
...
def EnergyEvo(dirName, A, B, n, M, N, step, z1, z2):
    ...
    for j in range(n):
        ax.plot(arg, asw[j], linewidth=3, label='{} core'.format(j))
    ax.plot(arg, asw[n], linewidth=3, color='black', label='single core')
...
def PowerEvo(dirName, A, B, n, M, N, z1, z2):
    ...
    for j in range(1, n + 1):
        ax0.plot(arg, asw_highest[j], linewidth=3, label='{} core'.format(j - 1))
    ax0.plot(arg, asw_highest[0], linewidth=3, color='black', label='single core')
    ...
    arg = np.linspace(z1, z2, N)
    for j in range(1, n + 1):
        ax1.plot(arg, asw_central[j], linewidth=3, label='{} core'.format(j - 1))
    ax1.plot(arg, asw_central[0], linewidth=3, color='black', label='single core')
...
def OutputSignal(dirName, A, B, n, t1, t2, z1, z2, N, M, outZ):
    ...
    for j in range(n):
        ...
        if j > 2:
            ...
            m += 1
        ax = axs[m, k]
        ax.set_title('input-output comparison at z={}cm, {} core'.format(actArg, j))
        ...
        ax.plot(arg, outputSingleCore, color='green', linewidth=3, label='output, single core')
        ax.plot(arg, outputMultiCore_j, color='blue', linewidth=3, label='output, {} core'.format(j))
    ...
    for j in range(n):
        ...
        ax.semilogy(arg, outputSingleCore, color='green', linewidth=3, label='output, single core')
        ax.semilogy(arg, outputMultiCore_j, color='blue', linewidth=3, label='output, {} core'.format(j))
        ax.legend(fontsize='12', facecolor='white', loc='upper right')
...
def PeakPowerCoord(dirName, highestPowEvo, A, B, n, step, M, N, z1, z2, t1, t2):
    ...
    for j in range(n + 1):
        ...
        if j > 2:
            ...
            m += 1
        current_title = 'peak coordinates EVO, {} core'.format(j)
        max_color = 'blue'
        all_color = 'green'
        if j == n:
            current_title = 'peak coordinates EVO, single core'
            max_color = 'yellow'
            all_color = 'red'
            m = 0
            k = 0
...
def PowerField(A, B, n, N, M, z1, z2, t1, t2, dirName):
    ...
    for j in range(n + 1):
        ...
        if j > 2:
            ...
            m += 1
        field = abs(B) ** 2
        current_title = 'signal power, single core'
        if j < n:
            field = abs(A[j]) ** 2
            current_title = 'signal power, {} core'.format(j)
        if j == n:
            m = 0
            k = 0
        ax = axs[m, k]
...
def CentralPhase(dirName, A, B, n, N, M, z1, z2):
    ...
    for j in range(n):
        ...
        if j > 2:
            ...
            m += 1
        ax = axs[m, k]
        ax.set_title('phase at t=0, {} core'.format(j))
        ...
        ax.set_xlim(left=z1, right=z2)
        ax.scatter(arg, asw[j], s=5, c='b', label='phase at {} core'.format(j))
        if choose[j] == criteria and sum(abs(A[j][0])) > 1:
            ax.scatter(arg, asw_single, s=5, c='g', label='phase at single core')
        ax.legend(fontsize='12', facecolor='white', loc='upper right')
    axs[0, 0].set_visible(False)
    name = 'фаза_при_t=0'
    path = dirName + '/' + name
...
def InstantFrequencyField(A, B, n, N, M, z1, z2, t1, t2, step, dirName, smoothBorder=50):
    ...
    for j in range(n + 1):
        ...
        if j > 2:
            ...
            m += 1
        field = IFreq_single
        current_title = 'instant frequency [THz], single core'
        if j < n:
            field = IFreq[j]
            current_title = 'instant frequency [THz], {} core'.format(j)
        if j == n:
            m = 0
            k = 0
        ax = axs[m, k]
...
def CentralPhaseChirp(dirName, IFreq, IFreq_single, A, B, n, z1, z2, t1, t2, N, M, way):
    ...
    for j in range(n):
        ...
        if j > 2:
            ...
            m += 1
        ax = axs[m, k]
        ax.set_title('chirp, {} core'.format(j))
        ...
        ax.set_xlim(left=z1, right=z2)
        ax.plot(arg, stuff, color='blue', linewidth=3, label='chirp at {} core'.format(j))
        if choose[j] == criteria and sum(abs(A[j][0])) > 1:
            ax.plot(arg, asw_single, color='green', linewidth=3, label='chirp at single core')
...
def HalfPeakWidth(dirName, highestPowEvo, A, B, n, step, N, M, z1, z2, t1):
    ...
    for j in range(n):
        ...
        if j > 2:
            ...
            m += 1
        ax = axs[m, r]
        ax.set_title('slice at half height, {} core'.format(j))
        ax.set_xlabel('time, fs')
        ax.set_ylabel('z, cm')
        # ax.xaxis.set_major_locator(ticker.MultipleLocator(200))
        # ax.xaxis.set_minor_locator(ticker.MultipleLocator(100))
        ...
        for k in indexes:
            if k == indexes[-1]:
                ...
                ax.scatter((-1) * asw_simple_single[k], arg[k], s=5, color='gray', label='slice, single core')
                continue
            ax.scatter(asw_hard[j][k], [arg[k]] * len(asw_hard[j][k]), s=5, c='r')
            ax.scatter(asw_simple[j][k], arg[k], s=5, color='r')
            ax.scatter((-1) * asw_simple[j][k], arg[k], s=5, color='r')  ###################
